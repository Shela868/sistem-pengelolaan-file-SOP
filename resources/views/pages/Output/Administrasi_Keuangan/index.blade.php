...
<x-breadcrumb :values="[__('Adminstrasi Dan Keuangan'), __('Output')]">
    <a href="{{ route('Output.Administrasi_Keuangan.create') }}" class="btn btn-primary">{{ __('menu.general.create') }}</a>
</x-breadcrumb>
                        ...
                        <button class="btn p-0" type="button" id="dropdown-{{ $out->type }}-{{ $out->id }}"
                            ...
                            <div class="dropdown-menu dropdown-menu-end"
                                ...
                                <a class="dropdown-item"
                                   href="{{  route('Output.Administrasi_Keuangan.edit', ['Administrasi_Keuangan' => $out->id]) }}">{{ __('menu.general.edit') }}</a>
                                <form action="{{ route('Output.Administrasi_Keuangan.destroy', $out->id) }}" class="d-inline"
                                      method="post">
                                    @csrf
                                    @method ('DELETE')
                                    <span
