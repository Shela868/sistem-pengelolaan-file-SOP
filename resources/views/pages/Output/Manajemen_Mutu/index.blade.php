...
<x-breadcrumb :values="[__('Manajemen Mutu'), __('Output')]">
    <a href="{{ route('Output.Manajemen_Mutu.create') }}" class="btn btn-primary">{{ __('menu.general.create') }}</a>
</x-breadcrumb>
                        ...
                        <button class="btn p-0" type="button" id="dropdown-{{ $out->type }}-{{ $out->id }}"
                            ...
                            <div class="dropdown-menu dropdown-menu-end"
                                ...
                                <a class="dropdown-item"
                                   href="{{  route('Output.Manajemen_Mutu.edit', ['Manajemen_Mutu' => $out->id]) }}">{{ __('menu.general.edit') }}</a>
                                <form action="{{ route('Output.Manajemen_Mutu.destroy', $out->id) }}" class="d-inline"
                                      method="post">
                                    @csrf
                                    @method ('DELETE')
                                    <span
