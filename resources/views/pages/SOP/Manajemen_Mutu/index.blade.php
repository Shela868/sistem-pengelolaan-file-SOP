...
<x-breadcrumb :values="[__('Manajemen Mutu'), __('SOP')]">
    <a href="{{ route('SOP.Manajemen_Mutu.create') }}" class="btn btn-primary">{{ __('menu.general.create') }}</a>
</x-breadcrumb>
        ...
        <div class="card-header pb-0">
            <div class="d-flex justify-content-between flex-column flex-sm-row">
                <div class="card-title">
                  <h5 class="mb-0 fw-bold" style="white-space: normal;">
                     {{ $sop->id }} | {{ $sop->perihal_SOP }}
                    </h5>
                        ...
                        <button class="btn p-0" type="button" id="dropdown-{{ $sop->type }}-{{ $sop->id }}"
                            ...
                            <div class="dropdown-menu dropdown-menu-end"
                                ...
                                <a class="dropdown-item"
                                   href="{{ route('SOP.Manajemen_Mutu.edit', $sop->id) }}">{{ __('menu.general.edit') }}</a>
                                <form action="{{ route('SOP.Manajemen_Mutu.destroy', $sop) }}" class="d-inline"
                                      method="post">
                                    @csrf
                                    @method ('DELETE')
                                    <span
